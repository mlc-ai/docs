...
#
# Usage: docker_bash.sh [-i|--interactive] [--net=host] [-t|--tty]
#          [--mount MOUNT_DIR] [--repo-mount-point REPO_MOUNT_POINT]
#          [--dry-run] [--name NAME] [--privileged]
#          <DOCKER_IMAGE_NAME> [--] [COMMAND]
#
# Usage: docker_bash.sh <CONTAINER_NAME>
#     Starts an interactive session
#
# Usage2: docker_bash.sh [-i] <CONTAINER_NAME> [COMMAND]
...
while (( $# )); do
    case "$1" in
        ...
        -*|--*)
            ...
            echo "  command (e.g. docker_bash.sh ci_gpu -- build -j2)" >&2
            show_usage >&2
            exit 1
            ;;
